...
import { SuggestionModel } from "../Utilities/model/Suggestion.model";
...
function App(): JSX.Element {
  ...
  const [suggestions, setSuggestions] = useState<SuggestionModel[] | string[]>(
    []
  );
  ...
  //handleClick
  const handleClick = async (item: SuggestionModel) => {
    // setLoading("loading")
  ...
  return (
    <div className="w-screen min-h-screen flex flex-col items-center bg-gradient-to-r from-[#000428] to-[#004e92]">
      <div className="card w-full h-full md:w-7/12 md:h-auto min-h-[400px] p-4 md:mt-8  border border-gray-600 rounded-2xl relative flex flex-col">
          ...
          {togle && suggestions && (
            <div className="w-full  bg-white rounded-md overflow-hidden absolute z-10">
              {suggestions?.map((item, index) =>
                typeof item === "string" ? (
                  <p className="block w-full text-start p-2 text-md font-medium tracking-wide text-black/50">
                    {item}
                  </p>
                ) : (
                  <button
